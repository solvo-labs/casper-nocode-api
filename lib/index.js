...
const getStakePool = async (contractHash, accountHash, client) => {
  ...
  stake.notified = await contract.queryContractData(["notified"]);
  stake.owner = uint32ArrayToHex(await contract.queryContractData(["owner"]));
  stake.token = uint32ArrayToHex((await contract.queryContractData(["token"])).data);
  stake.amIOwner = stake.owner === accountHash;

  if (stake.fixed_apr.toNumber() === 0) {
  ...
  try {
    stake.liquidity = await contract.queryContractData(["liquidity"]);
  } catch {
    stake.liquidity = undefined;
  }

  try {
    stake.apr = await contract.queryContractData(["apr"]);
  } catch {
    stake.apr = undefined;
  }
  ...
  try {
    ...
    stake.my_balance = undefined;
  }

  try {
    const myClaimed = await contract.queryContractDictionary("claimed_dict", accountHash);
    stake.my_claimed = myClaimed;
  } catch {
    stake.my_claimed = undefined;
  }

  try {
    stake.total_reward = await contract.queryContractData(["total_reward"]);
  } catch {
    stake.total_reward = undefined;
  }
